package io.github.taoguan.luaj;

import io.github.taoguan.luaj.vm.LuaInstruction;
import io.github.taoguan.luaj.vm.OpCode;

import static io.github.taoguan.luaj.vm.LuaInstruction.*;

/**
 * Extension of {@link io.github.taoguan.luaj.LuaFunction} which executes lua bytecode.
 * <p>
 * A {@link LuaClosure} is a combination of a {@link io.github.taoguan.luaj.Prototype}
 * and a {@link io.github.taoguan.luaj.LuaValue} to use as an environment for execution.
 * Normally the {@link io.github.taoguan.luaj.LuaValue} is a {@link io.github.taoguan.luaj.Globals} in which case the environment
 * will contain standard lua libraries. 
 * 
 * <p>
 * There are three main ways {@link LuaClosure} instances are created:
 * <ul> 
 * <li>Construct an instance using {@link #LuaClosure(io.github.taoguan.luaj.Prototype, io.github.taoguan.luaj.LuaValue)}</li>
 * <li>Construct it indirectly by loading a chunk via {@link io.github.taoguan.luaj.Globals#load(java.io.Reader, String)}
 * <li>Execute the lua bytecode {@link OpCode#CLOSURE} as part of bytecode processing
 * </ul>
 * <p>
 * To construct it directly, the {@link io.github.taoguan.luaj.Prototype} is typically created via a compiler such as
 * {@link io.github.taoguan.luaj.compiler.LuaC}:
 * <pre> {@code
 * String script = "print( 'hello, world' )";
 * InputStream is = new ByteArrayInputStream(script.getBytes());
 * Prototype p = LuaC.instance.compile(is, "script");
 * LuaValue globals = JsePlatform.standardGlobals();
 * LuaClosure f = new LuaClosure(p, globals);
 * f.call();
 * }</pre> 
 * <p>
 * To construct it indirectly, the {@link io.github.taoguan.luaj.Globals#load(java.io.Reader, String)} method may be used:
 * <pre> {@code
 * Globals globals = JsePlatform.standardGlobals();
 * LuaFunction f = globals.load(new StringReader(script), "script");
 * LuaClosure c = f.checkclosure();  // This may fail if LuaJC is installed.
 * c.call();
 * }</pre>
 * <p>
 * In this example, the "checkclosure()" may fail if direct lua-to-java-bytecode 
 * compiling using LuaJC is installed, because no LuaClosure is created in that case
 * and the value returned is a {@link io.github.taoguan.luaj.LuaFunction} but not a {@link LuaClosure}.
 * <p> 
 * Since a {@link LuaClosure} is a {@link io.github.taoguan.luaj.LuaFunction} which is a {@link io.github.taoguan.luaj.LuaValue},
 * all the value operations can be used directly such as:
 * <ul>
 * <li>{@link io.github.taoguan.luaj.LuaValue#call()}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#call(io.github.taoguan.luaj.LuaValue)}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#invoke()}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#invoke(io.github.taoguan.luaj.Varargs)}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#method(String)}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#method(String, io.github.taoguan.luaj.LuaValue)}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#invokemethod(String)}</li>
 * <li>{@link io.github.taoguan.luaj.LuaValue#invokemethod(String, io.github.taoguan.luaj.Varargs)}</li>
 * <li> ...</li> 
 * </ul>
 * @see io.github.taoguan.luaj.LuaValue
 * @see io.github.taoguan.luaj.LuaFunction
 * @see io.github.taoguan.luaj.LuaValue#isclosure()
 * @see io.github.taoguan.luaj.LuaValue#checkclosure()
 * @see io.github.taoguan.luaj.LoadState
 * @see io.github.taoguan.luaj.Globals#compiler
 */
public class LuaClosure extends io.github.taoguan.luaj.LuaFunction {
	private static final io.github.taoguan.luaj.UpValue[] NOUPVALUES = new io.github.taoguan.luaj.UpValue[0];
	
	public final io.github.taoguan.luaj.Prototype p;

	public io.github.taoguan.luaj.UpValue[] upValues;
	
	final io.github.taoguan.luaj.Globals globals;
	
	/** Create a closure around a Prototype with a specific environment.
	 * If the prototype has upvalues, the environment will be written into the first upvalue.
	 * @param p the Prototype to construct this Closure for. 
	 * @param env the environment to associate with the closure.
	 */
	public LuaClosure(io.github.taoguan.luaj.Prototype p, io.github.taoguan.luaj.LuaValue env) {
		this.p = p;
		this.initupvalue1(env);
		globals = env instanceof io.github.taoguan.luaj.Globals ? (io.github.taoguan.luaj.Globals) env: null;
	}
	
	public void initupvalue1(io.github.taoguan.luaj.LuaValue env) {
		if (p.upvalues == null || p.upvalues.length == 0)
			this.upValues = NOUPVALUES;
		else {
			this.upValues = new io.github.taoguan.luaj.UpValue[p.upvalues.length];
			this.upValues[0] = new io.github.taoguan.luaj.UpValue(new io.github.taoguan.luaj.LuaValue[] {env}, 0);
		}
	}

	
	public boolean isclosure() {
		return true;
	}
	
	public LuaClosure optclosure(LuaClosure defval) {
		return this;
	}

	public LuaClosure checkclosure() {
		return this;
	}
	
	public String tojstring() {
		return "function: " + p.toString();
	}
	
	private io.github.taoguan.luaj.LuaValue[] getNewStack() {
		int max = p.maxstacksize;
		io.github.taoguan.luaj.LuaValue[] stack = new io.github.taoguan.luaj.LuaValue[max];
		System.arraycopy(NILS, 0, stack, 0, max);
		return stack;
	}
	
	public final io.github.taoguan.luaj.LuaValue call() {
		io.github.taoguan.luaj.LuaValue[] stack = getNewStack();
		return execute(stack,NONE).arg1();
	}

	public final io.github.taoguan.luaj.LuaValue call(io.github.taoguan.luaj.LuaValue arg) {
		io.github.taoguan.luaj.LuaValue[] stack = getNewStack();
		switch ( p.numparams ) {
		default: stack[0]=arg; return execute(stack,NONE).arg1();
		case 0: return execute(stack,arg).arg1();
		}
	}
	
	public final io.github.taoguan.luaj.LuaValue call(io.github.taoguan.luaj.LuaValue arg1, io.github.taoguan.luaj.LuaValue arg2) {
		io.github.taoguan.luaj.LuaValue[] stack = getNewStack();
		switch ( p.numparams ) {
		default: stack[0]=arg1; stack[1]=arg2; return execute(stack,NONE).arg1();
		case 1: stack[0]=arg1; return execute(stack,arg2).arg1();
		case 0: return execute(stack,p.is_vararg!=0? varargsOf(arg1,arg2): NONE).arg1();
		}
	}

	public final io.github.taoguan.luaj.LuaValue call(io.github.taoguan.luaj.LuaValue arg1, io.github.taoguan.luaj.LuaValue arg2, io.github.taoguan.luaj.LuaValue arg3) {
		io.github.taoguan.luaj.LuaValue[] stack = getNewStack();
		switch ( p.numparams ) {
		default: stack[0]=arg1; stack[1]=arg2; stack[2]=arg3; return execute(stack,NONE).arg1();
		case 2: stack[0]=arg1; stack[1]=arg2; return execute(stack,arg3).arg1();
		case 1: stack[0]=arg1; return execute(stack,p.is_vararg!=0? varargsOf(arg2,arg3): NONE).arg1();
		case 0: return execute(stack,p.is_vararg!=0? varargsOf(arg1,arg2,arg3): NONE).arg1();
		}
	}

	public final io.github.taoguan.luaj.Varargs invoke(io.github.taoguan.luaj.Varargs varargs) {
		return onInvoke(varargs).eval();
	}
	
	public final io.github.taoguan.luaj.Varargs onInvoke(io.github.taoguan.luaj.Varargs varargs) {
		io.github.taoguan.luaj.LuaValue[] stack = getNewStack();
		for ( int i=0; i<p.numparams; i++ )
			stack[i] = varargs.arg(i+1);
		return execute(stack,p.is_vararg!=0? varargs.subargs(p.numparams+1): NONE);
	}
	
	protected io.github.taoguan.luaj.Varargs execute(io.github.taoguan.luaj.LuaValue[] stack, io.github.taoguan.luaj.Varargs varargs ) {
		// loop through instructions
		int i,a,b,c,pc=0,top=0;
		io.github.taoguan.luaj.LuaValue o;
		io.github.taoguan.luaj.Varargs v = NONE;
		int[] code = p.code;
		io.github.taoguan.luaj.LuaValue[] k = p.k;
		
		// upvalues are only possible when closures create closures
		// TODO: use linked list.
		io.github.taoguan.luaj.UpValue[] openups = p.p.length>0? new io.github.taoguan.luaj.UpValue[stack.length]: null;
		
		// allow for debug hooks
		if (globals != null && globals.debuglib != null)
			globals.debuglib.onCall( this, varargs, stack ); 

		// process instructions
		try {
			for (; true; ++pc) {
				if (globals != null && globals.debuglib != null)
					globals.debuglib.onInstruction( pc, v, top ); 
				
				// pull out instruction
				i = code[pc];
				OpCode opCode = getOpCode(i);
				a = getA(i);
				
				// process the op code
				switch ( opCode ) {
				
				case MOVE:/*	A B	R(A):= R(B)					*/
					stack[a] = stack[getB(i)];
					continue;
					
				case LOADK:/*	A Bx	R(A):= Kst(Bx)					*/
					stack[a] = k[getBx(i)];
					continue;

				case LOADKX:/*	A Bx	R(A) := Kst(extra arg)				*/
					stack[a] = k[getAx(code[++pc])];
					continue;
					
				case LOADBOOL:/*	A B C	R(A):= (Bool)B: if (C) pc++			*/
	                stack[a] = (getB(i)!=0)? io.github.taoguan.luaj.LuaValue.TRUE: io.github.taoguan.luaj.LuaValue.FALSE;
	                if (getC(i) != 0)
	                    ++pc; /* skip next instruction (if C) */
	                continue;
	
				case LOADNIL: /*	A B	R(A):= ...:= R(A+B):= nil			*/
					for ( b=getB(i); b-->=0; )
						stack[a++] = io.github.taoguan.luaj.LuaValue.NIL;
					continue;
					
				case GETUPVAL: /*	A B	R(A):= UpValue[B]				*/
	                stack[a] = upValues[getB(i)].getValue();
	                continue;
					
				case GETTABUP: /*	A B C	R(A) := UpValue[B][RK(C)]			*/
					stack[a] = upValues[getB(i)].getValue().get((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
	                
				case GETTABLE: /*	A B C	R(A):= R(B)[RK(C)]				*/
	                stack[a] = stack[getB(i)].get((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case SETTABUP: /*	A B C	UpValue[A][RK(B)] := RK(C)			*/
	                upValues[a].getValue().set(((b=getB(i))>0xff? k[b&0x0ff]: stack[b]), (c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case SETUPVAL: /*	A B	UpValue[B]:= R(A)				*/
					upValues[getB(i)].setValue(stack[a]);
					continue;
					
				case SETTABLE: /*	A B C	R(A)[RK(B)]:= RK(C)				*/
					stack[a].set(((b=getB(i))>0xff? k[b&0x0ff]: stack[b]), (c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case NEWTABLE: /*	A B C	R(A):= {} (size = B,C)				*/
					stack[a] = new io.github.taoguan.luaj.LuaTable(getB(i),getC(i));
					continue;
					
				case SELF: /*	A B C	R(A+1):= R(B): R(A):= R(B)[RK(C)]		*/
					stack[a+1] = (o = stack[getB(i)]);
					stack[a] = o.get((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case ADD: /*	A B C	R(A):= RK(B) + RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).add((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case SUB: /*	A B C	R(A):= RK(B) - RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).sub((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case MUL: /*	A B C	R(A):= RK(B) * RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).mul((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case DIV: /*	A B C	R(A):= RK(B) / RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).div((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case IDIV: /*	A B C	R(A) := RK(B) // RK(C)			*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).idiv((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case MOD: /*	A B C	R(A):= RK(B) % RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).mod((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;
					
				case POW: /*	A B C	R(A):= RK(B) ^ RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).pow((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case BAND: /*	A B C	R(A) := RK(B) & RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).band((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case BOR: /*	A B C	R(A) := RK(B) | RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).bor((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case BXOR: /*	A B C	R(A) := RK(B) ~ RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).bxor((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case SHL: /*	A B C	R(A) := RK(B) << RK(C)			*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).shl((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case SHR: /*	A B C	R(A) := RK(B) >> RK(C)				*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).shr((c=getC(i))>0xff? k[c&0x0ff]: stack[c]);
					continue;

				case BNOT: /*	A B C	R(A) := ~R(B)			*/
					stack[a] = ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).bnot();
					continue;
					
				case UNM: /*	A B	R(A):= -R(B)					*/
					stack[a] = stack[getB(i)].neg();
					continue;
					
				case NOT: /*	A B	R(A):= not R(B)				*/
					stack[a] = stack[getB(i)].not();
					continue;
					
				case LEN: /*	A B	R(A):= length of R(B)				*/
					stack[a] = stack[getB(i)].len();
					continue;
					
				case CONCAT: /*	A B C	R(A):= R(B).. ... ..R(C)			*/
					b = getB(i);
					c = getC(i);
					{
						if ( c > b+1 ) {
							io.github.taoguan.luaj.Buffer sb = stack[c].buffer();
							while ( --c>=b ) 
								sb.concatTo(stack[c]);
							stack[a] = sb.value();
						} else {
							stack[a] = stack[c-1].concat(stack[c]);
						}
					}
					continue;
					
				case JMP: /*	sBx	pc+=sBx					*/
					pc  += getSBx(i);
					if (a > 0) {
						for (--a, b = openups.length; --b>=0; )
							if (openups[b] != null && openups[b].index >= a) {
								openups[b].close();
								openups[b] = null;
							}
					}
					continue;
					
				case EQ: /*	A B C	if ((RK(B) == RK(C)) ~= A) then pc++		*/
					if ( ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).eq_b((c=getC(i))>0xff? k[c&0x0ff]: stack[c]) != (a!=0) )
						++pc;
					continue;
					
				case LT: /*	A B C	if ((RK(B) <  RK(C)) ~= A) then pc++  		*/
					if ( ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).lt_b((c=getC(i))>0xff? k[c&0x0ff]: stack[c]) != (a!=0) )
						++pc;
					continue;
					
				case LE: /*	A B C	if ((RK(B) <= RK(C)) ~= A) then pc++  		*/
					if ( ((b=getB(i))>0xff? k[b&0x0ff]: stack[b]).lteq_b((c=getC(i))>0xff? k[c&0x0ff]: stack[c]) != (a!=0) )
						++pc;
					continue;
					
				case TEST: /*	A C	if not (R(A) <=> C) then pc++			*/
					if ( stack[a].toboolean() != (getC(i)!=0) )
						++pc;
					continue;
					
				case TESTSET: /*	A B C	if (R(B) <=> C) then R(A):= R(B) else pc++	*/
					/* note: doc appears to be reversed */
					if ( (o=stack[getB(i)]).toboolean() != (getC(i)!=0) )
						++pc;
					else
						stack[a] = o; // TODO: should be sBx? 
					continue;
					
				case CALL: /*	A B C	R(A), ... ,R(A+C-2):= R(A)(R(A+1), ... ,R(A+B-1)) */
					switch ( i & (LuaInstruction.MASK_B | LuaInstruction.MASK_C) ) {
					case (1<< LuaInstruction.POS_B) | (0<< LuaInstruction.POS_C): v=stack[a].invoke(NONE); top=a+v.narg(); continue;
					case (2<< LuaInstruction.POS_B) | (0<< LuaInstruction.POS_C): v=stack[a].invoke(stack[a+1]); top=a+v.narg(); continue;
					case (1<< LuaInstruction.POS_B) | (1<< LuaInstruction.POS_C): stack[a].call(); continue;
					case (2<< LuaInstruction.POS_B) | (1<< LuaInstruction.POS_C): stack[a].call(stack[a+1]); continue;
					case (3<< LuaInstruction.POS_B) | (1<< LuaInstruction.POS_C): stack[a].call(stack[a+1],stack[a+2]); continue;
					case (4<< LuaInstruction.POS_B) | (1<< LuaInstruction.POS_C): stack[a].call(stack[a+1],stack[a+2],stack[a+3]); continue;
					case (1<< LuaInstruction.POS_B) | (2<< LuaInstruction.POS_C): stack[a] = stack[a].call(); continue;
					case (2<< LuaInstruction.POS_B) | (2<< LuaInstruction.POS_C): stack[a] = stack[a].call(stack[a+1]); continue;
					case (3<< LuaInstruction.POS_B) | (2<< LuaInstruction.POS_C): stack[a] = stack[a].call(stack[a+1],stack[a+2]); continue;
					case (4<< LuaInstruction.POS_B) | (2<< LuaInstruction.POS_C): stack[a] = stack[a].call(stack[a+1],stack[a+2],stack[a+3]); continue;
					default:
						b = getB(i);
						c = getC(i);
						v = stack[a].invoke(b>0? 
							varargsOf(stack, a+1, b-1): // exact arg count
							varargsOf(stack, a+1, top-v.narg()-(a+1), v));  // from prev top 
						if ( c > 0 ) {
							v.copyto(stack, a, c-1);
							v = NONE;
						} else {
							top = a + v.narg();
							v = v.dealias();
						}
						continue;
					}
					
				case TAILCALL: /*	A B C	return R(A)(R(A+1), ... ,R(A+B-1))		*/
					switch ( i & LuaInstruction.MASK_B ) {
					case (1<< LuaInstruction.POS_B): return new io.github.taoguan.luaj.TailcallVarargs(stack[a], NONE);
					case (2<< LuaInstruction.POS_B): return new io.github.taoguan.luaj.TailcallVarargs(stack[a], stack[a+1]);
					case (3<< LuaInstruction.POS_B): return new io.github.taoguan.luaj.TailcallVarargs(stack[a], varargsOf(stack[a+1],stack[a+2]));
					case (4<< LuaInstruction.POS_B): return new io.github.taoguan.luaj.TailcallVarargs(stack[a], varargsOf(stack[a+1],stack[a+2],stack[a+3]));
					default:
						b = getB(i);
						v = b>0? 
							varargsOf(stack,a+1,b-1): // exact arg count
							varargsOf(stack, a+1, top-v.narg()-(a+1), v); // from prev top 
						return new io.github.taoguan.luaj.TailcallVarargs( stack[a], v );
					}
					
				case RETURN: /*	A B	return R(A), ... ,R(A+B-2)	(see note)	*/
					b = getB(i);
					switch ( b ) {
					case 0: return varargsOf(stack, a, top-v.narg()-a, v); 
					case 1: return NONE;
					case 2: return stack[a]; 
					default:
						return varargsOf(stack, a, b-1);
					}
					
				case FORLOOP: /*	A sBx	R(A)+=R(A+2): if R(A) <?= R(A+1) then { pc+=sBx: R(A+3)=R(A) }*/
					{
			            io.github.taoguan.luaj.LuaValue limit = stack[a + 1];
						io.github.taoguan.luaj.LuaValue step  = stack[a + 2];
						io.github.taoguan.luaj.LuaValue idx   = step.add(stack[a]);
			            if (step.gt_b(0)? idx.lteq_b(limit): idx.gteq_b(limit)) {
		                    stack[a] = idx;
		                    stack[a + 3] = idx;
		                    pc += getSBx(i);
			            }
					}
					continue;
					
				case FORPREP: /*	A sBx	R(A)-=R(A+2): pc+=sBx				*/
					{
						io.github.taoguan.luaj.LuaValue init  = stack[a].checknumber("'for' initial value must be a number");
						io.github.taoguan.luaj.LuaValue limit = stack[a + 1].checknumber("'for' limit must be a number");
						io.github.taoguan.luaj.LuaValue step  = stack[a + 2].checknumber("'for' step must be a number");
						stack[a] = init.sub(step);
						stack[a + 1] = limit;
						stack[a + 2] = step;
						pc += getSBx(i);
					}
					continue;

				case TFORCALL: /* A C	R(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));	*/
					v = stack[a].invoke(varargsOf(stack[a+1],stack[a+2]));
					c = getC(i);
					while (--c >= 0)
						stack[a+3+c] = v.arg(c+1);
					v = NONE;
					continue;

				case TFORLOOP: /* A sBx	if R(A+1) ~= nil then { R(A)=R(A+1); pc += sBx */
					if (!stack[a+1].isnil()) { /* continue loop? */
						stack[a] = stack[a+1];  /* save control varible. */
						pc += getSBx(i);
					}
					continue;
					
				case SETLIST: /*	A B C	R(A)[(C-1)*FPF+i]:= R(A+i), 1 <= i <= B	*/
					{
		                if ( (c=getC(i)) == 0 )
		                    c = getAx(code[++pc]);
		                int offset = (c-1) * LuaInstruction.LFIELDS_PER_FLUSH;
		                o = stack[a];
		                if ( (b=getB(i)) == 0 ) {
		                    b = top - a - 1;
		                    int m = b - v.narg(); 
		                	int j=1;
		                	for ( ;j<=m; j++ )
		                    	o.set(offset+j, stack[a + j]);
		                	for ( ;j<=b; j++ )
		                    	o.set(offset+j, v.arg(j-m));
		                } else {
		                    o.presize( offset + b );
		                    for (int j=1; j<=b; j++)
		                    	o.set(offset+j, stack[a + j]);
		                }
					}
					continue;
					
				case CLOSURE: /*	A Bx	R(A):= closure(KPROTO[Bx])	*/
					{
						io.github.taoguan.luaj.Prototype newp = p.p[getBx(i)];
						LuaClosure ncl = new LuaClosure(newp, globals);
						io.github.taoguan.luaj.Upvaldesc[] uv = newp.upvalues;
						for ( int j=0, nup=uv.length; j<nup; ++j ) {
							if (uv[j].instack)  /* upvalue refes to local variable? */
								ncl.upValues[j] = findupval(stack, uv[j].idx, openups);
							else  /* get upvalue from enclosing function */
								ncl.upValues[j] = upValues[uv[j].idx];
						}
						stack[a] = ncl;
					}
					continue;
					
				case VARARG: /*	A B	R(A), R(A+1), ..., R(A+B-1) = vararg		*/
					b = getB(i);
					if ( b == 0 ) {
						top = a + (b = varargs.narg());
						v = varargs;
					} else { 
						for ( int j=1; j<b; ++j )
							stack[a+j-1] = varargs.arg(j);
					}
					continue;				

				case EXTRAARG:
					throw new IllegalArgumentException("Uexecutable opcode: OP_EXTRAARG");

				default:
					throw new IllegalArgumentException("Illegal opcode: " + (i & 0x3f));
				}
			}
		} catch ( io.github.taoguan.luaj.LuaError le ) {
			if (le.traceback == null)
				processErrorHooks(le, p, pc);
			throw le;
		} catch ( Exception e ) {
			io.github.taoguan.luaj.LuaError le = new io.github.taoguan.luaj.LuaError(e);
			processErrorHooks(le, p, pc);
			throw le;
		} finally {
			if ( openups != null )
				for ( int u=openups.length; --u>=0; )
					if ( openups[u] != null )
						openups[u].close();
			if (globals != null && globals.debuglib != null)
				globals.debuglib.onReturn();
		}
	}

	/**
	 *  Run the error hook if there is one
	 *  @param msg the message to use in error hook processing. 
	 * */
	String errorHook(String msg, int level) {
		if (globals == null ) return msg;
		final io.github.taoguan.luaj.LuaThread r = globals.running;
		if (r.errorfunc == null)
			return globals.debuglib != null?
					msg + "\n" + globals.debuglib.traceback(level):
					msg;
		final io.github.taoguan.luaj.LuaValue e = r.errorfunc;
		r.errorfunc = null;
		try {
			return e.call( io.github.taoguan.luaj.LuaValue.valueOf(msg) ).tojstring();
		} catch ( Throwable t ) {
			return "error in error handling";
		} finally {
			r.errorfunc = e;
		}
	}

	private void processErrorHooks(io.github.taoguan.luaj.LuaError le, io.github.taoguan.luaj.Prototype p, int pc) {
		le.fileline = (p.source != null? p.source.tojstring(): "?") + ":" 
			+ (p.lineinfo != null && pc >= 0 && pc < p.lineinfo.length? String.valueOf(p.lineinfo[pc]): "?");
		le.traceback = errorHook(le.getMessage(), le.level);
	}
	
	private io.github.taoguan.luaj.UpValue findupval(io.github.taoguan.luaj.LuaValue[] stack, short idx, io.github.taoguan.luaj.UpValue[] openups) {
		final int n = openups.length;
		for (int i = 0; i < n; ++i)
			if (openups[i] != null && openups[i].index == idx)
				return openups[i];
		for (int i = 0; i < n; ++i)
			if (openups[i] == null)
				return openups[i] = new io.github.taoguan.luaj.UpValue(stack, idx);
		error("No space for upvalue");
		return null;
	}

	protected io.github.taoguan.luaj.LuaValue getUpvalue(int i) {
		return upValues[i].getValue();
	}
	
	protected void setUpvalue(int i, io.github.taoguan.luaj.LuaValue v) {
		upValues[i].setValue(v);
	}

	public String name() {
		return "<"+p.shortsource()+":"+p.linedefined+">";
	}
	
	
}
