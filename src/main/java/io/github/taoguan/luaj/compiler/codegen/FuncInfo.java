...
import io.github.taoguan.luaj.vm.FPB;
import io.github.taoguan.luaj.vm.OpCode;

import java.util.*;

import static io.github.taoguan.luaj.compiler.lexer.TokenKind.*;
import static io.github.taoguan.luaj.vm.LuaInstruction.MAXARG_sBx;
...
class FuncInfo {
    ...
    Map<LuaValue, Integer> constants = new HashMap<>();
    private List<List<Integer>> breaks = new ArrayList<>();
    private List<List<Integer>> continues = new ArrayList<>();
    ...
    int indexOfConstant(LuaValue k) {
    ...
    void enterScope(boolean isloop, boolean breakable, boolean continuable) {
        scopeLv++;
        if (breakable) {
        ...
        if (continuable) {
            continues.add(new ArrayList<>());
        } else {
            continues.add(null);
        }
    }

    void exitScope(int endPC, int continuePC) {
    ...
    void fixContinueJmpSbx(int endBlockPc){
        List<Integer> pendingContinueJmps = continues.remove(continues.size() - 1);

        if (pendingContinueJmps != null) {
            for (int pc : pendingContinueJmps) {
                if(endBlockPc > pc){
                    fixSbx(pc, endBlockPc - pc);
                }else {
                    fixSbx(pc, pc - endBlockPc -1);
                }
            }
        }
    }
    ...
    int addLocVar(String name, int startPC) {
        ...
        if(newVar.prev != null && newVar.prev.scopeLv == scopeLv){
            if(newVar.prev.endPC <= 0){
                newVar.prev.endPC = startPC - 1;
            }
        }
    ...
    void addContinueJmp(int pc) {
        for (int i = scopeLv; i >= 0; i--) {
            if (continues.get(i) != null) { // breakable
                continues.get(i).add(pc);
                return;
            }
        }
    ...
    // r[a] = kst[bx]
    void emitLoadK(int line, int a, LuaValue k) {
    ...
    void emitSetList(int line, int a, int b, int c) {
        if(c <= 0xFF){
            ...
            emitABC(line, OpCode.SETLIST, a, b, 0);
            emitAx(line, OpCode.EXTRAARG, c);
        }
    ...
    int emitTForCall(int line, int a, int c) {
