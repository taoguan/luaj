...
import static io.github.taoguan.luaj.compiler.codegen.BlockProcessor.*;
import static io.github.taoguan.luaj.compiler.codegen.BlockProcessor.processBlock;
import static io.github.taoguan.luaj.compiler.codegen.ExpProcessor.*;
import static io.github.taoguan.luaj.compiler.codegen.ExpProcessor.ARG_REG;
import static io.github.taoguan.luaj.compiler.codegen.ExpProcessor.expToOpArg;
import static io.github.taoguan.luaj.compiler.codegen.ExpProcessor.processExp;
import static io.github.taoguan.luaj.compiler.codegen.ExpProcessor.processFuncCallExp;
import static io.github.taoguan.luaj.compiler.codegen.ExpProcessor.processFuncDefExp;
...
class StatProcessor {
    ...
    static void processStat(FuncInfo fi, Stat node) {
        if (node instanceof FuncCallStat) {
            ...
            processContinueStat(fi, (ContinueStat) node);
    ...
    private static void processContinueStat(FuncInfo fi, ContinueStat node) {
    ...
    */
    private static void processWhileStat(FuncInfo fi, WhileStat node) {
        ...
        fi.enterScope(true, true, true);
        processBlock(fi, node.getBlock());
        fi.closeOpenUpvals(node.getBlock().getLastLine());
        int continuePC = fi.pc();
        fi.emitJmp(node.getBlock().getLastLine(), 0, pcBeforeExp-fi.pc()-1);
        fi.exitScope(fi.pc(), continuePC);
    ...
    */
    private static void processRepeatStat(FuncInfo fi, RepeatStat node) {
        ...
        fi.exitScope(fi.pc() + 1, continuePC);
    }
    ...
    */
    ...
    private static void processForInStat(FuncInfo fi, ForInStat node) {
        ...
        int pcTForCall = fi.emitTForCall(line, rGenerator, node.getNameList().size());
    ...
    private static void processAssignStat(FuncInfo fi, AssignStat node) {
        ...
        for (int i = 0; i < node.getVarList().size(); i++) {
            ...
            if (exp instanceof TableAccessExp) {
                ...
                if (fi.slotOfLocVar(name) < 0 && fi.indexOfUpval(name) < 0) {
                    // global var
                    kRegs[i] = -1;
                    if (fi.indexOfConstant(LuaString.valueOf(name)) > 0xFF) {
                        kRegs[i] = fi.allocReg();
                    }
                }
            }
        ...
        for (int i = 0; i < node.getVarList().size(); i++) {
            ...
            if (a >= 0) {
                if (kRegs[i] < 0) {
                    b = 0x100 + fi.indexOfConstant(LuaString.valueOf(varName));
