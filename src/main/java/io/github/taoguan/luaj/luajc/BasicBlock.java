...
public class BasicBlock {
	...
	public static void visitBranches(io.github.taoguan.luaj.Prototype p, BranchVisitor visitor ) {
		...
		for ( int i=0; i<n; i++ ) {
			int ins = code[i];
			switch ( LuaInstruction.getOpCode( ins ) ) {
			case LOADBOOL:
				if ( 0 == LuaInstruction.getC(ins) )
					break;
				if ( LuaInstruction.getOpCode(code[i+1]) == OpCode.JMP  )
			...
			case TESTSET:
				...
				sbx = LuaInstruction.getSBx(code[i+1]);
				++i;
				j = i + sbx + 1;
