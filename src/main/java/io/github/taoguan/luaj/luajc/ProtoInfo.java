...
public class ProtoInfo {
	...
	private VarInfo[][] findVariables() {
		...
		for ( int bi=0; bi<blocklist.length; bi++ ) {
			...
			for ( int pc=b0.pc0; pc<=b0.pc1; pc++ ) {
				...
				OpCode op = LuaInstruction.getOpCode(ins);
				...
				case SHR: /*	A B C	R(A) := RK(B) >> RK(C)				*/
					a = LuaInstruction.getA( ins );
					b = LuaInstruction.getB( ins );
					c = LuaInstruction.getC( ins );
					if (!LuaInstruction.ISK(b)) v[b][pc].isreferenced = true;
					if (!LuaInstruction.ISK(c)) v[c][pc].isreferenced = true;
					v[a][pc] = new VarInfo(a,pc);
					break;
	...
	private void findUpvalues() {
		...
		for ( int pc=0; pc<n; pc++ ) {
			if ( LuaInstruction.getOpCode(code[pc]) == OpCode.CLOSURE ) {
				int bx = LuaInstruction.getBx(code[pc]);
		...
		for ( int pc=0; pc<n; pc++ ) {
			if ( LuaInstruction.getOpCode(code[pc]) == OpCode.SETUPVAL )
				upvals[LuaInstruction.getB(code[pc])].rw = true;
		}
	}
